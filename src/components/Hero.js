...
const Hero = () => {
  ...
  const fetchSettings = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000/api'}/site-settings`);
    ...
    <section className="hero-section">
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 2, pointerEvents: 'none' }}>
        <DotField
          ...
          dotSpacing={isMobile ? 18 : 14}
          bulgeStrength={isMobile ? 50 : 67}
          glowRadius={isMobile ? 120 : 160}
          sparkle={false}
          waveAmplitude={0}
          cursorRadius={isMobile ? 300 : 500}
          cursorForce={0.1}
          bulgeOnly
