...
class Media {
  ...
  createShader() {
    ...
    if (this.video) {
      ...
      videoEl.addEventListener('loadedmetadata', () => {
        this.program.uniforms.uImageSizes.value = [videoEl.videoWidth, videoEl.videoHeight];
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const baseWidth = isMobile ? 450 : 700;
  ...
  useEffect(() => {
    ...
    const fetchGallery = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000/api'}/gallery-videos`);
